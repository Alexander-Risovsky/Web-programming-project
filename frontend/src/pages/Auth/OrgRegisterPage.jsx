...
export default function OrgRegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast("error", err.message || "Ошибка регистрации организации");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="relative h-screen overflow-hidden flex flex-col">
      ...
      <header className="relative z-10 flex flex-col items-center justify-center w-full gap-2 px-4 py-3 border-b sm:flex-row sm:gap-3 sm:px-6 sm:py-4 border-slate-200/30 bg-white/10 backdrop-blur-sm">
        ...
        <span className="text-sm font-medium text-center sm:text-lg text-slate-600 sm:text-left">
          <span className="whitespace-nowrap">Регистрация новой студорганизации</span>
        </span>
      </header>

      <div className="relative z-10 flex-1 min-h-0 flex items-center justify-center px-4 py-4">
        <div className="relative w-full max-w-xl p-5 overflow-hidden border-2 shadow-xl bg-white/90 backdrop-blur-sm sm:p-7 lg:p-8 rounded-2xl lg:rounded-3xl border-slate-200/50 animate-scale-in group -translate-y-2">
          ...
          <div className="relative z-10">
            <div className="mb-4 text-center sm:mb-6">
              <h1 className="mb-2 text-2xl font-bold text-transparent sm:text-3xl bg-gradient-to-r from-primary via-purple-600 to-indigo-600 bg-clip-text">
                <span className="whitespace-nowrap">Регистрация организации</span>
              </h1>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="w-full transition-all duration-300 bg-white border-2 shadow-sm rounded-xl border-slate-200 text-slate-900 hover:shadow-md overflow-hidden focus-within:ring-2 focus-within:ring-primary focus-within:border-primary">
                  <textarea
                    value={form.description}
                    onChange={handleChange("description")}
                    placeholder="Кратко расскажите об организации"
                    className="w-full px-4 py-3 bg-transparent text-slate-900 focus:outline-none h-20 resize-none overflow-y-auto"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="relative w-full py-3 sm:py-3.5 bg-gradient-to-r from-primary via-purple-600 to-indigo-600 text-white rounded-xl font-semibold transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden group"
              >
