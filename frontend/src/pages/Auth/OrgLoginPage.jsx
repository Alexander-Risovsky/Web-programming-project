...
export default function OrgLoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await loginOrg({ username: login, password: pass });
      if (!result?.success) {
      ...
      <div className="relative z-10 flex-1 min-h-0 flex items-center justify-center px-4 py-6">
        <div className="relative w-full max-w-md p-6 overflow-hidden border-2 shadow-xl bg-white/90 backdrop-blur-sm sm:p-8 lg:p-10 rounded-2xl lg:rounded-3xl border-slate-200/50 animate-scale-in group">
          ...
          <div className="relative z-10">
            ...
            <form onSubmit={handleSubmit} noValidate className="flex flex-col space-y-4 sm:space-y-5">
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-slate-700">Логин или e-mail</label>
                <div className="relative">
            ...
            <p className="mt-4 text-sm text-center text-slate-600">
              Нет аккаунта?{" "}
              <Link to="/org/register" className="font-semibold text-primary hover:underline">
